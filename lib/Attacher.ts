...
class Attacher extends EventEmitter {
    ...
    */
     ...
     */
    async attach() {
        let exists: boolean = false,    // Does container even exists
            attached: boolean = false,  // Is the pseudo-tty attached to the running container
        ...
        logger.stdout?.on("data", (d) => {
            if(!attached && exists) { return this.emit("inc", { out: d }); }    // Only start sending data to user once it is confirmed that container does exist.
            logs += d;
        });
        ...
        try { await p(exec)(`docker ps | grep ${this.opts.containerName}`) } catch(e) {
            logger.kill();
        ...
        this.process.onData(async (e) => {
            if(e.substr(0, 25) === "Error: No such container:") { return; }                         // Handles race condition that compiler has already been destroyed upon attach
            if(e === "You cannot attach to a stopped container, start it first\r\n") { return; }    // Handles race condition that compiler was in the midst of getting destroyed upon attach
            if(!attached) { attached = true; logger.kill(); this.emit("attached"); }                // Sets attached to true so that {logger} is no longer needed.
            this.emit("inc", { out: e });
        });
        ...
        this.checkInterval = setInterval(() => {
            ...
            exec(`docker ps | grep ${this.opts.containerName}`, (err, out) => {
                ...
                if ((!out || timedOut) && !sentDone) {
                    ...
                    return this.emit("done", { err: "", out: "", timedOut: timedOut });
                }
            });
        }, 100);
    }
